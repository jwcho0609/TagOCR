...
class scanner(QRunnable):
    ...
    def run(self):
        ...
        if self.flag == 'pass':
            ...
            polyPer, spanPer, cotPer = [100, 0, 0]
            datatools.addItem(polyPer, spanPer, cotPer)
            scanalgo.resetCount = 0
        elif self.flag == 'span':
            polyPer, spanPer, cotPer = [0, 100, 0]
            datatools.addItem(polyPer, spanPer, cotPer)
            scanalgo.resetCount = 0
        elif self.flag == 'cot':
            polyPer, spanPer, cotPer = [0, 0, 100]
            datatools.addItem(polyPer, spanPer, cotPer)
            scanalgo.resetCount = 0
...
class MainWindow(QWidget):
    ...
    def __init__(self):
        ...
        self.unkBtn.setFixedSize(62, 80)
        self.unkBtn.setFont(QFont('Comic sans', 14))

        self.polyBtn = QPushButton('Polyester', self)
        self.polyBtn.setFixedSize(62, 80)
        self.polyBtn.setFont(QFont('Comic sans', 14))

        self.spanBtn = QPushButton('Spandex', self)
        self.spanBtn.setFixedSize(62, 80)
        self.spanBtn.setFont(QFont('Comic sans', 14))

        self.cotBtn = QPushButton('Cotton', self)
        self.cotBtn.setFixedSize(62, 80)
        self.cotBtn.setFont(QFont('Comic sans', 14))
        ...
        hboxBot.addWidget(self.polyBtn)
        hboxBot.addWidget(self.spanBtn)
        hboxBot.addWidget(self.cotBtn)
        ...
        self.polyBtn.clicked.connect(self.polyTag)
        self.spanBtn.clicked.connect(self.spanTag)
        self.cotBtn.clicked.connect(self.cotTag)
    ...
    def unkTag(self):
        ...
        self.polyBtn.setEnabled(False)
        self.spanBtn.setEnabled(False)
        self.cotBtn.setEnabled(False)

        scanThread = scanner(self.status, self.tools, self.scanBtn, self.poly, self.span, self.cot, self.w, self.unkBtn, 'unk')
        c.pool.start(scanThread)
        self.unkBtn.setEnabled(True)
        self.polyBtn.setEnabled(True)
        self.spanBtn.setEnabled(True)
        self.cotBtn.setEnabled(True)
    ...
    def polyTag(self):
        ...
        scanThread = scanner(self.status, self.tools, self.scanBtn, self.poly, self.span, self.cot, self.w, self.unkBtn, 'poly')
        c.pool.start(scanThread)
        self.unkBtn.setEnabled(True)
        self.polyBtn.setEnabled(True)
        self.spanBtn.setEnabled(True)
    ...
    def spanTag(self):
        ...
        scanThread = scanner(self.status, self.tools, self.scanBtn, self.poly, self.span, self.cot, self.w, self.unkBtn, 'span')
        c.pool.start(scanThread)
        self.unkBtn.setEnabled(True)
        self.polyBtn.setEnabled(True)
        self.spanBtn.setEnabled(True)
    ...
    def cotTag(self):
        ...
        scanThread = scanner(self.status, self.tools, self.scanBtn, self.poly, self.span, self.cot, self.w, self.unkBtn, 'cot')
        c.pool.start(scanThread)
    ...
    def scan(self):
        ...
        self.cotBtn.setEnabled(False)
        self.scanBtn.setEnabled(False)
        scanThread = scanner(self.status, self.tools, self.scanBtn, self.poly, self.span, self.cot, self.w, self.unkBtn, 'pass')
        c.pool.start(scanThread)
